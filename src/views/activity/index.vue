...
export default {
        ...
        },
        {
          ...
          render: (h, row) => {
            return (
              <dl>
                <dt>开始时间</dt>
                <dd>{formatDate(row.started_time)}</dd>
                <dt>结束时间</dt>
                <dd>{formatDate(row.ended_time)}</dd>
              </dl>
            )
          }
        },
  ...
  methods: {
    ...
    getData ({ offset, limit }) {
      return getActivityList({ offset, limit })
    },
    add () {
      this.$router.push('/activity/0')
    },
    modify (data) {
    ...
    delData (data) {
      return delActivity(data.id).then(() => {
